...
for dataset in "${datasets[@]}"; do
    ...
    python -m benchmark --boost_fr --dataset "$dataset" --oracle --pred oracle_bin --noise_type bin --dump_file --output_root_dir stat > "logs/benchmark/oracle/${dataset}_bin.log" 2>&1 &
    pids+=($!)
done
...
python scripts/aggregate_results.py --name bin --results_dir stat
