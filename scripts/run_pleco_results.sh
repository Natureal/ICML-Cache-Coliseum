...
output_root_dir="${OUTPUT_ROOT_DIR:-results/pleco}"
boost_preds_dir="${BOOST_PREDS_DIR:-boost_traces}"
log_dir="${LOG_DIR:-logs/benchmark/pleco_results}"
...
for dataset in "${datasets[@]}"; do
    ...
    echo "Running PLECO benchmark for dataset=$dataset"
    python -m benchmark \
        --dataset "$dataset" \
        --real \
        --pred pleco \
    ...
    generated_result="$output_root_dir/$dataset/1/pleco.csv"
