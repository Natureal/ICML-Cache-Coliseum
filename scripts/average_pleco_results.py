#!/usr/bin/env python3
"""Average benchmark CSV columns across datasets, grouped by algorithm."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Average every numeric result column for each algorithm."
    )
    parser.add_argument(
        "--input-dir",
        type=Path,
        default=Path("results/pleco"),
        help="Directory containing per-dataset CSV files (default: results/pleco).",
    )
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("results/pleco/average.csv"),
        help="Output summary CSV path (default: results/pleco/average.csv).",
    )
    parser.add_argument(
        "--precision",
        type=int,
