...
for dataset in "${datasets[@]}"; do
    ...
    python -m benchmark --boost --boost_fr --dataset "$dataset" --real --pred popu --dump_file --output_root_dir stat > "logs/benchmark/popu/${dataset}.log" 2>&1 &
    pids+=($!)
done
...
python scripts/aggregate_results.py --name popu --results_dir stat
